  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4">
      <form onSubmit={handleSubmit} className="relative transition-all duration-500">
        <div className="relative flex flex-col bg-[#0D0D0D] border border-white/10 rounded-[26px] shadow-[0_10px_40px_rgba(0,0,0,0.5)] focus-within:border-white/20 transition-all duration-300 overflow-hidden">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3 animate-in fade-in slide-in-from-bottom-2 duration-1000">
          {SEARCH_SUGGESTIONS.map((tag) => (
            <button
              ...
              className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-xs font-semibold text-zinc-400 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all"
            >
              {tag}
            </button>
          ))}
