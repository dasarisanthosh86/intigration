...
const Dashboard = () => {
    ...
    const advanceToNextStep = async (stepIndex?: number, forcedRepoUrl?: string) => {
        ...
        if (nextIndex < AGENTS.length) {
            ...
            try {
                ...
                const prevMessage = [...messages].reverse().find(m => m.agentOutput);
                const query = prevMessage ? prevMessage.agentOutput : "";

                console.log(`[Dashboard] Advancing to step ${nextIndex + 1} with context length: ${query.length}`);
                ...
                if (result.status === 'success') {
                    ...
                    setMessages(prev => [...prev, {
                        ...
                        fileId: result.file_id || 'latest', // Keep fileId for potential future use
    ...
    // PDF Generation moved to utils/pdfGenerator

    return (
        <div className="min-h-screen bg-black flex overflow-hidden relative">
            <Sidebar />

            <main className="flex-1 ml-72 p-8 pt-32 overflow-y-auto overflow-x-hidden">
                            ...
                            <div className="bg-gradient-to-r from-emerald-500 to-teal-600 p-8 rounded-[2.5rem] flex items-center justify-between shadow-[0_20px_50px_rgba(16,185,129,0.2)]">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <a
                                        ...
                                        className={`px-6 py-3 rounded-xl text-xs font-bold transition-all shadow-lg flex items-center gap-2 ${githubRepoUrl ? 'bg-white text-emerald-600' : 'bg-white/10 text-white/50 cursor-not-allowed'}`}
                        ...
                        {AGENTS.map((agent, index) => {
                            ...
                            return (
                                <AgentCard
                                    ...
                                    showDownload={agentStatuses[agent.id] === 'success'}
                                    ...
                                    onDownload={() => {
                                        if (agent.id === 'architecture') generatePDF("System Architecture Report", "Scale-ready cloud architecture blueprint including frontend React setup, Python FastAPI backend, and PostgreSQL persistence layer.");
                                        else if (agent.id === 'impact') generatePDF("Impact Analysis Report", "Comprehensive evaluation of technical debt, business scalability, and security posture for the proposed architecture.");
                                        else generatePDF("Source Code Summary", "Verified production-quality codebase generated with strict type-safety and modular architecture patterns.");
                                    }}
                                />
                            );
                        })}
