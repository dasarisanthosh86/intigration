...
def seed_antigravity():
    print("Seeding Antigravity Enterprise SDLC Platform...")
    
    db = SessionLocal()
    try:
        # 1. Create Admin User if not exists
        admin_email = "admin@antigravity.ai"
        admin_user = db.query(User).filter(User.email == admin_email).first()
        if not admin_user:
            user_in = UserCreate(
                email=admin_email,
                password="antigravity_admin_2025",
        ...
        # 2. Define Antigravity Specialized Agents (Aligned with 7-Agent SDLC)
        agents_data = [
            {
                "name": "UI/UX Designer",
...
Finalize the project for production."""
        ...
        db.commit()
        print("Antigravity agents synced successfully.")
