...
└─────────────────┘    └─────────────────┘    └─────────────────┘
    ...
    def generate_architecture_pdf(self, architecture, github_url="", prd_included=False, repo_analysis=None, prd_content=None, prd_file_path=None) -> str:
        ...
        try:
            logger.info("Adding architecture diagram section...")
            story.extend(self._create_architecture_diagram_section())
            story.append(PageBreak())
        except Exception as e:
            logger.error(f"Failed to add architecture diagram: {e}")
        
        # Add layered data flow diagram
        try:
            logger.info("Adding layered dataflow section...")
            story.extend(self._create_layered_dataflow_section())
        except Exception as e:
            logger.error(f"Failed to add layered dataflow: {e}")
        
        # Build the PDF
        try:
            logger.info(f"Building PDF with {len(story)} elements...")
            doc.build(story)
            logger.info(f"PDF successfully built at: {filepath}")
